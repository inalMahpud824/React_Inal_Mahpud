...
export const FormProduct = () => {
  ...
  const deleteProduct = (id) => {
    const confirm = window.confirm('apakah data dengan no ' + id + ' ingin di hapus ?');
    if (!confirm){
      return;
    }
            ...
            {products.length > 0 &&
              products.map((product) => (
                <tr key={product.no}>
                  ...
                  <td>{product.price}</td>
                  <td><button className="btn btn-danger" onClick={() => deleteProduct(product.no)}>Delete</button> <button onClick={() => editProduct(product.no)} className="btn btn-primary">Edit</button></td>
                </tr>
              ))}
          </tbody>
